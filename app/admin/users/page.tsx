...
// Detectar tipo de convenio o categoría especial según role y tipo_acceso
function getTipoUsuario(u: Profile): string {
    if (u.role === 'abogado_colegio') return 'abogados'
    const ta = (u.tipo_acceso || '').toLowerCase()
    const ae = (u.actividad_empresa || '').toLowerCase()
    if (ta.includes('contador') || ta.includes('cpce') || ae.includes('contador') || ae.includes('cpce')) return 'contadores'
    if (ta.includes('oame') || ae.includes('oame')) return 'oame'
    if (u.role === 'user') return 'usuarios'
    return 'otros'
}
...
export default function UsersPage() {
  ...
  async function load() {
      ...
      // Cargar membresías activas para detectar miembros activos
  ...
  function exportarClientesExcel() {
      ...
      const resumenData = [
              ...
              ['Abogados Colegio', conteos.abogados],
              ['Contadores CPCE', conteos.contadores],
              ['Miembros Activos', conteos.miembros],
              ['OAME', conteos.oame],
              ['Admins', conteos.admins],
              ['', ''],
  ...
  return (
        <div>
              <div className="flex items-center justify-between mb-6">
                      <h1 className="text-2xl font-bold" style={{ color: '#0a2744' }}>Usuarios registrados</h1>h1>
                      <div className="flex items-center gap-3">
                                <span className="text-sm text-gray-400">{users.length} en total</span>span>
                    ...
                    <button key={f.key} onClick={() => setFiltro(f.key)}
                      ...
                      {f.label}
                                <span className={`ml-1.5 text-xs px-1.5 py-0.5 rounded-full ${filtro === f.key ? 'bg-white/20 text-white' : 'bg-gray-100 text-gray-500'}`}>
                                  {conteos[f.key as keyof typeof conteos] ?? 0}
                                </span>span>
                    </button>button>
                  ))}
              </div>div>
          ...
          {loading ? (
                  <div className="flex items-center justify-center h-40 text-gray-400">Cargando...</div>div>
                ) : filtrados.length === 0 ? (
                  <div className="bg-white rounded-xl border border-gray-200 p-12 text-center text-gray-400">Sin usuarios en esta categoría</div>div>
                ) : (
                                ...
                                <div key={u.id}
                                              ...
                                              <div className="flex-1 min-w-0">
                                                              <div className="flex items-center gap-2 flex-wrap">
                                                                                <p className="font-semibold text-gray-900">{u.full_name || '—'}</p>p>
                                                                ...
                                                                {memberships.has(u.id) && (
                                                                      <span className="text-xs bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full">💎 Miembro activo</span>span>
                                                                                )}
                                                                {u.foto_dni_url && <span className="text-xs bg-green-100 text-green-600 px-2 py-0.5 rounded-full">📄 DNI</span>span>}
                                                                {u.foto_matricula_url && <span className="text-xs bg-blue-100 text-blue-600 px-2 py-0.5 rounded-full">⚖️ Matrícula</span>span>}
                                                              </div>div>
                                                              <div className="flex gap-3 mt-0.5 text-xs text-gray-400 flex-wrap">
                                                                                <span>{u.email}</span>span>
                                                                {u.telefono && <span>📞 {u.telefono}</span>span>}
                                                                {u.dni_cuil && <span>DNI: {u.dni_cuil}</span>span>}
                                                                {u.actividad_empresa && <span>· {u.actividad_empresa}</span>span>}
                                                              </div>div>
                                              </div>div>
                                
                                              <div className="text-right shrink-0">
                                                              <p className="text-xs text-gray-400">{new Date(u.created_at).toLocaleDateString('es-AR', { day: 'numeric', month: 'short', year: 'numeric' })}</p>p>
                                                              <p className="text-xs text-blue-500 mt-1">Ver ficha →</p>p>
                                              </div>div>
                                </div>div>
                              ))}
                  </div>div>
                  ...
                  <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setSelected(null)}>
                            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                                        ...
                                        <div className="flex items-center justify-between p-5 rounded-t-2xl" style={{ background: '#0a2744' }}>
                                                      <div className="flex items-center gap-3">
                                                                      ...
                                                                      <div>
                                                                                        <p className="text-white font-semibold">{selected.full_name || '—'}</p>p>
                                                                                        <p className="text-blue-200 text-xs">{selected.email}</p>p>
                                                                      </div>div>
                                                      </div>div>
                                                      <button onClick={() => setSelected(null)} className="text-blue-200 hover:text-white text-2xl leading-none">×</button>button>
                                        </div>div>
                                        ...
                                        <div className="p-5 space-y-5">
                                          ...
                                          {memberships.has(selected.id) && (
                                    <div className="rounded-xl px-4 py-3 text-sm flex items-center gap-2 bg-amber-50 border border-amber-200 text-amber-700">
                                                      💎 Miembro activo con membresía vigente
                                    </div>div>
                                                      )}
                                        
                                                      <div>
                                                                      <p className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-3">Datos personales</p>p>
                    ...
                    { label: 'País / Provincia', value: selected.pais_provincia },
                                                            ...
                                                            <div key={label} className="flex justify-between text-sm border-b border-gray-50 pb-2">
                                                                                  <span className="text-gray-500">{label}</span>span>
                                                                                  <span className="font-medium text-gray-800 text-right max-w-[55%]">{value}</span>span>
                                                            </div>div>
                                                          ) : null)}
                                                                      </div>div>
                                                      </div>div>
                                                      ...
                                                      <div>
                                                                      <p className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-3">Actividad y facturación</p>p>
                                                                      <div className="space-y-2">
                                                                        {[
                    ...
                    { label: 'Facturación a nombre de', value: selected.facturacion_nombre },
                                    ...
                                    <div>
                                                      <p className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-3">Documentos</p>p>
                                                      <div className="grid grid-cols-2 gap-3">
                                                            ...
                                                            <a href={selected.foto_dni_url} target="_blank" rel="noopener noreferrer" className="block">
                                                                                    <div className="rounded-xl overflow-hidden border border-gray-200 aspect-video relative group">
                                                                                                              ...
                                                                                                              <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-colors flex items-center justify-center">
                                                                                                                                          <span className="text-white text-xs font-semibold opacity-0 group-hover:opacity-100">Abrir foto</span>span>
                                                                                                                </div>div>
                                                                                      </div>div>
                                                                                    <p className="text-xs text-center text-gray-500 mt-1">📄 DNI (frente)</p>p>
                                                            </a>a>
                                                                          )}
                                                        {selected.foto_matricula_url && (
                                                            <a href={selected.foto_matricula_url} target="_blank" rel="noopener noreferrer" className="block">
                                                                                    ...
                                                                                    <p className="text-xs text-center text-gray-500 mt-1">⚖️ Matrícula</p>p>
                                                            </a>a>
                                                                          )}
                                                      </div>div>
                                    </div>div>
                                    ...
                                    <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-sm text-amber-700">
                                                      ...
                                                      <div>
                                                                      <p className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-3">Cambiar rol</p>p>
