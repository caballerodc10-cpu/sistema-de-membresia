...
<button onClick={() => { setShowResetModal(false); setResetMsg(''); setResetError(''); setResetEmail('') }} className="absolute top-4 right-4 text-gray-400 hover:text-gray-600 transition-colors">
...
<input type="email" value={resetEmail} onChange={e => setResetEmail(e.target.value)} required placeholder="tu@email.com" className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 text-gray-900 bg-white shadow-sm text-sm" />
</div>
{resetError && <div className="rounded-xl bg-red-50 border border-red-100 px-4 py-3 text-red-600 text-sm">{resetError}</div>}
<button type="submit" disabled={resetLoading} className="login-btn w-full font-bold py-3 rounded-xl text-sm disabled:opacity-60" style={{ color: '#0a2744' }}>{resetLoading ? 'Enviando...' : 'Enviar link de reseteo'}</button>
</form>
)}
</div>
</div>
...
.glass-input { background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.18); color: #fff; transition: border-color 0.2s, background 0.2s; }
...
<input type="email" value={email} onChange={e => setEmail(e.target.value)} required autoComplete="email" className="glass-input w-full px-4 py-3 rounded-xl text-sm" placeholder="tu@email.com" />
</div>
<div>
<div className="flex items-center justify-between mb-1.5">
<label className="block text-sm font-semibold text-blue-200">Contraseña</label>
<button type="button" onClick={() => setShowResetModal(true)} className="text-xs font-medium transition-colors hover:text-white" style={{ color: '#c5e84a' }}>¿Olvidaste tu contraseña?</button>
</div>
<div className="relative">
<input type={showPassword ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} required autoComplete="current-password" className="glass-input w-full px-4 py-3 pr-11 rounded-xl text-sm" placeholder="••••••••" />
...
<Link href="/members/login" className="flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-xs font-semibold transition-all" style={{ background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.12)', color: 'rgba(255,255,255,0.7)' }}><span>👥</span> Portal Members</Link>
<a href="https://wa.me/5493794899843?text=Hola%20Oruga!%20Quiero%20reservar%20una%20sala" target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-xs font-semibold transition-all" style={{ background: 'rgba(37,211,102,0.12)', border: '1px solid rgba(37,211,102,0.25)', color: '#4ade80' }}><span>💬</span> WhatsApp</a>
</div>
</div>
<p className="text-center text-xs mt-5" style={{ color: 'rgba(255,255,255,0.3)' }}>¿No tenés cuenta?{' '}<Link href="/register" className="font-semibold hover:text-white transition-colors" style={{ color: '#c5e84a' }}>Registrarse</Link></p>
</div>
<p className="text-center text-xs mt-4" style={{ color: 'rgba(255,255,255,0.25)' }}>© {new Date().getFullYear()} Oruga Cowork · Corrientes</p>
</div>
</div>
</div>
)
