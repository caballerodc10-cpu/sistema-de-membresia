...
export default function HomePage() {
  ...
  // Detectar si hoy es sábado (6) o domingo (0)
        ...
        <div className="space-y-6">
              ...
              <div className="relative rounded-2xl overflow-hidden" style={{ minHeight: 220, background: '#0a2744' }}>
                      ...
                      <div className="relative z-10 p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4" style={{ minHeight: 220 }}>
                                <div className="flex flex-col justify-center">
                                            ...
                                            <p className="text-sm font-medium mb-1" style={{ color: '#c5e84a' }}>{fechaHoy}</p>p>
                                            <h1 className="text-2xl font-bold text-white">
                                              {userName ? `Hola, ${userName.split(' ')[0]}` : 'Bienvenido'}
                                            </h1>h1>
                                            <p className="text-blue-200 text-sm mt-1">Buenos Aires 678 · Corrientes Capital</p>p>
                                </div>div>
                                <div className="text-right shrink-0">
                                            <p className="text-5xl font-bold text-white tabular-nums">{hora}</p>p>
                                            <p className="text-blue-200 text-xs mt-1">hora local</p>p>
                                </div>div>
                      </div>div>
              </div>div>
          ...
          {!isAdmin && esFinDeSemana && (
                  <a
                            ...
                            >
                            <div>
                                        <p className="text-white font-bold text-sm">📅 Reservas de fin de semana</p>p>
                                        <p className="text-blue-200 text-xs mt-0.5">Los sábados y domingos consultá disponibilidad por WhatsApp</p>p>
                            </div>div>
                            <div className="flex items-center gap-2 px-4 py-2 rounded-xl font-semibold text-sm shrink-0"
          ...
          {/* Disponibilidad ahora */}
              <div>
                ...
                {loading ? (
                    <div className="h-32 flex items-center justify-center text-gray-400">Cargando...</div>div>
                  ) : (
                                                    ...
                                                    const color = ROOM_COLORS[sala.name] || '#616161'
                                                                    return (
                                                                                      <div key={sala.id}
                                                                                                        ...
                                                                                                        <p className="font-bold text-gray-900 text-sm leading-tight">{sala.name}</p>p>
                                                                                                        <p className="text-xs text-gray-400 flex items-center gap-1">
                                                                                                                            <span>&#x1F465;</span>span> hasta {sala.capacity} personas
                                                                                                          </p>p>
                                                                                        {!libre && sala.ocupadaHasta && (
                                                                                                                                <p className="text-xs font-medium" style={{ color: '#dc2626' }}>Libera {formatHora(sala.ocupadaHasta)}</p>p>
                                                                                                        )}
                                                                                        {libre && sala.proximaReserva && (
                                                                                                                                <p className="text-xs font-medium text-amber-500">Ocupada desde {formatHora(sala.proximaReserva)}</p>p>
                                                                                                        )}
                                                                                        {libre && !sala.proximaReserva && (
                                                                                                                                <p className="text-xs font-medium text-emerald-600">Todo el día libre</p>p>
                                                                                                        )}
                                                                                        </div>div>
                                                                                    )
                      })}
          ...
          {!loading && (
                  <div className="grid grid-cols-2 gap-3">
                            <div
                                        ...
                                        >
                                        <p className="text-4xl font-black" style={{ color: '#15803d' }}>{disponibles.length}</p>p>
                                        <p className="text-sm font-medium mt-1" style={{ color: '#166534' }}>salas disponibles</p>p>
                            </div>div>
                            <div
                                          className="rounded-2xl p-5 text-center"
                                          style={{
                                        ...
                                        >
                                        <p className="text-4xl font-black" style={{ color: '#dc2626' }}>{ocupadas.length}</p>p>
                                        <p className="text-sm font-medium mt-1" style={{ color: '#991b1b' }}>salas ocupadas</p>p>
                            </div>div>
                  </div>div>
              )}
          ...
          {/* Accesos rapidos */}
              <div>
                      <h2 className="text-lg font-bold mb-3" style={{ color: '#0a2744' }}>Accesos rapidos</h2>h2>
                        ...
                        : { href: '/membership', label: 'Membresia', icon: '&#x1F48E;', desc: 'Tu plan actual', accent: '#f59e0b' },
                                  ...
                                  <Link key={item.href} href={item.href}
                                                ...
                                                <div
                                                                ...
                                                                />
                                                <p className="font-bold text-gray-900 text-sm">{item.label}</p>p>
                                                <p className="text-xs text-gray-400 mt-0.5">{item.desc}</p>p>
                                  </Link>Link>
                                ))}
                      </div>div>
              </div>div>
          ...
          {isAdmin && (
                  <div className="rounded-2xl overflow-hidden border-2 shadow-sm" style={{ borderColor: '#c5e84a' }}>
                            <div className="px-5 py-3 flex items-center gap-2" style={{ background: '#0a2744' }}>
                                        <span className="text-xs font-bold px-2 py-0.5 rounded-full" style={{ background: '#c5e84a', color: '#0a2744' }}>&#x1F6E0; ADMIN</span>span>
                                        <p className="text-white font-semibold text-sm">Panel de administracion</p>p>
                            </div>div>
                    ...
                    { href: '/admin/users', label: 'Usuarios', icon: '&#x1F465;' },
                                                ...
                                                <Link key={item.href} href={item.href}
                                                                ...
                                                                <span className="text-xs font-medium text-gray-700">{item.label}</span>span>
                                                </Link>Link>
                                              ))}
                            </div>div>
                  </div>div>
          ...
          {!isAdmin && (
                  <a href="https://wa.me/5493794899843?text=Hola%20Oruga!%20Quiero%20consultar%20disponibilidad%20para%20reservar%20una%20sala"
                            ...
                            <div>
                                        <p className="text-white font-semibold">Queres reservar?</p>p>
                                        <p className="text-blue-200 text-sm">Escribinos por WhatsApp</p>p>
                            </div>div>
