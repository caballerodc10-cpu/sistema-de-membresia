...
export default function BookingModal({ initialDate, onClose, onSaved, isAdmin = false }: Props) {
  ...
  useEffect(() => {
    ...
    const fetchAdminUsers = async () => {
      ...
      if (membError || !membershipsData || membershipsData.length === 0) return
      // Step 2: get profiles for those user_ids
      const userIds = [...new Set(membershipsData.map((m: any) => m.user_id as string))]
      ...
      const users = userIds
        ...
        .map(uid => {
          const profile = profilesData?.find((p: any) => p.id === uid)
          const membership = membershipsData.find((m: any) => m.user_id === uid)
          return {
            id: uid,
            full_name: profile?.full_name || profile?.email || uid,
            email: profile?.email || '',
